...
export default function App() {
      ...
      ws.onmessage = (event) => {
        ...
        if (data.type === "init" || data.type === "update") {
          ...
          setPrivateOrder((prev) => {
            ...
            let max = Object.values(next).length
              ? Math.max(...Object.values(next))
              : -1;
            ...
            const validIds = new Set<number>(
              data.private_rooms.map((r: Private_Room) => r.id_private_room)
            );
          ...
          setSharedOrder((prev) => {
            ...
            const validIds = new Set<number>(
              data.work_rooms.map((r: Work_Room) => r.id_room)
            );
            Object.keys(next).forEach((k) => {
              const id = Number(k);
              if (!validIds.has(id)) delete next[id];
  ...
  useEffect(() => {
    function handleMouseMove(e: MouseEvent) {
      ...
      if (
        newWidth < window.innerWidth * 0.2 ||
  ...
  const getUsersInLocation = (locationId: number, type: WorkspaceType) => {
    return users.filter(
      (u: User) => u.currentLocation === locationId && u.locationType === type
    );
  };
  ...
  const shouldEnableVideoConference = (): boolean => {
    ...
    if (locationType === "shared" || locationType === "private") {
      const usersInCurrentWorkspace = getUsersInLocation(
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col">
          ...
          <div ref={panelRef} className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 px-4 py-2 bg-neutral-900 rounded-lg border border-neutral-800">
              {currentUser.imageURL && (
                <img
                  src={import.meta.env.VITE_API_BASE_URL + currentUser.imageURL}
                  alt={currentUser.user_name}
                  className="w-8 h-8 rounded-full object-cover"
                />
              )}
              <span className="stext-sm text-white">
      ...
      <div className="flex-1 flex overflow-hidden">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
                    {privateWorkspaces.map((room) => {
                      const owner = allUsers.find(
                        (u) => u.id_user === room.id_user
                      );

                      const usersInOffice = allUsers.filter(
                        (u) =>
                          u.currentLocation === room.id_private_room &&
                          u.locationType === "private"
                      );

                      const isOwner = currentUser.id_user === room.id_user;
                      const isCurrentUserHere =
                        currentUser.currentLocation === room.id_private_room &&
                        currentUser.locationType === "private";

                      const isOwnerInSharedSpace =
                        owner && owner.locationType === "shared";
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* ---------- ICONOS ---------- */}
                          <div className="flex gap-2 items-center justify-center mx-auto min-h-[48px]">
                                ...
                                >
                                  {user.imageURL ? (
                                    <img
                                      ...
                                      className="w-10 h-10 rounded-full border-2 border-slate-200 cursor-pointer object-cover"
                                    />
                                  ) : (
                                    <div
                                      onClick={() => handleTouchUser(user.user_name)}
                                ...
                                <div className="group relative">
                                  {owner.imageURL ? (
                                    <img
                                      ...
                                      alt={owner.user_name}
                                      className="w-12 h-12 rounded-full border-2 border-slate-200 cursor-pointer object-cover"
                  ...
                  {showCreateWorkspace && (
                    <div className="mt-4 flex gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setNewWorkspaceMaxUsers(e.target.value)
                        }
                        onKeyPress={(e) =>
                          e.key === "Enter" &&
                          createWorkSpace(newWorkspaceMaxUsers)
                        }
                      ...
                      <button
                        ...
                        disabled={
                          !newWorkspaceMaxUsers.trim() ||
                          parseInt(newWorkspaceMaxUsers) < 1
                        }
                ...
                <div className="p-6">
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    {sharedWorkspaces?.map((room) => {
                      const usersInSpace = getUsersInLocation(
                        room.id_room,
                        "shared"
                      );
                      const isCurrentUserHere =
                        currentUser.currentLocation === room.id_room &&
                        currentUser.locationType === "shared";
                                ...
                                >
                                  {user.imageURL ? (
                                    <img
                                      ...
                                      alt={user.user_name}
                                      className="w-12 h-12 rounded-full border-2 border-slate-200 hover:border-yellow-500 transition-colors cursor-pointer object-cover"
                                      onClick={() => handleTouchUser(user.user_name)}
                                    />
                                  ) : (
                                    <div className="w-12 h-12 rounded-full border-2 border-slate-200 bg-slate-300 flex items-center justify-center hover:border-yellow-500 transition-colors cursor-pointer flex-shrink-0"
                                      onClick={() => handleTouchUser(user.user_name)}
                                    >
                                      <span className="text-sm text-slate-600 font-medium">
        ...
        {/* Chat Panel */}
        <div
          className={`bg-white  border-l border-slate-200 flex`}
          style={{ position: "relative", width: `${chatWidth}px` }}
        >
